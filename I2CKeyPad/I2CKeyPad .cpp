...
uint8_t I2CKeyPad::_getKey4x4()
{
  ...
  if (cols == 0xF0)      return I2C_KEYPAD_NOKEY;
  else if (cols == 0xE0) key = 0;
  else if (cols == 0xD0) key = 1;
  else if (cols == 0xB0) key = 2;
  else if (cols == 0x70) key = 3;
  else return I2C_KEYPAD_FAIL;

  // 4 columns as input pull up, 4 rows as output
  uint8_t rows = _read(0x0F);
  // check if single line has gone low.
  if (rows == 0x0F)      return I2C_KEYPAD_NOKEY;
  else if (rows == 0x06) key += 0;
  else if (rows == 0x05) key += 4;
  else if (rows == 0x03) key += 8;
  else if (rows == 0x07) key += 12;
  else return I2C_KEYPAD_FAIL;
